...
import { Search, Barcode, ShoppingCart, Trash2, Plus, Minus, XCircle, DollarSign } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from "@/components/ui/dialog"
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table"
import type { CashSession, Sale, SaleItem, Product, ItemRemovalLog, SaleCancellationLog } from "@/lib/types"
import { findProductByCode, searchProductsByDescription, MOCK_PRODUCTS } from "@/lib/mock-products"
...
export function PdvSales({ session, onSaleComplete }: PdvSalesProps) {
  ...
  const [cancellationLogs, setCancellationLogs] = useState<SaleCancellationLog[]>([])
  ...
  const addOrUpdateItem = (product: Product) => {
    ...
    setCurrentSale((prevSale) => {
      const existingItemIndex = prevSale.items.findIndex((item) => item.product.id === product.id)
    ...
    <div className="h-screen flex flex-col bg-background">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">
                PDV - Venda {currentSale.status === "cancelled" ? "Cancelada" : "em Andamento"}
              </h1>
      ...
      <div className="flex-1 flex gap-4 p-4 overflow-hidden">
        ...
        <div className="w-96 space-y-4">
          ...
          <Card>
            ...
            <CardContent>
              <form onSubmit={handleCodeSubmit} className="space-y-3">
                ...
                />
                <Button type="submit" className="w-full h-10" disabled={currentSale.status === "cancelled"}>
                  Adicionar Produto
                </Button>
              </form>
              {errorMessage && (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="border rounded-md max-h-96 overflow-y-auto">
                      {searchResults.length > 0 ? (
                          ...
                          <TableBody>
                            {searchResults.map((product) => (
                              <TableRow key={product.id}>
                                ...
                                <TableCell>
                                  <Button size="sm" onClick={() => handleSelectSearchResult(product)}>
                                    Adicionar
                                  </Button>
                                </TableCell>
                              </TableRow>
                        ...
                        <div className="p-8 text-center text-muted-foreground">Digite para buscar produtos</div>
                      )}
                    </div>
                  </div>
                </DialogContent>
          ...
          {currentSale.status === "in-progress" && currentSale.items.length > 0 && (
              ...
              <div className="text-xs space-y-1 max-h-64 overflow-y-auto">
                {MOCK_PRODUCTS.map((product) => (
                  <div key={product.id} className="flex justify-between py-1 border-b">
                    ...
                    <span className="text-muted-foreground truncate ml-2">{product.description}</span>
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        <div className="flex-1 flex flex-col gap-4">
              ...
              {currentSale.items.length === 0 ? (
                <div className="h-full flex items-center justify-center text-muted-foreground">
                  <div className="text-center">
                    ...
                    <p className="text-sm">Escaneie ou digite o código do produto</p>
                  </div>
                </div>
              ) : (
                <Table>
                  ...
                  <TableBody>
                    {currentSale.items.map((item) => (
                      <TableRow key={item.id} className={currentSale.status === "cancelled" ? "opacity-50" : ""}>
                        <TableCell className="font-mono text-sm">{item.product.internalCode}</TableCell>
          ...
          {/* Totals */}
          <Card className={currentSale.status === "cancelled" ? "bg-muted" : "bg-primary text-primary-foreground"}>
            <CardContent className="pt-6">
              <div className="space-y-2">
                <div className="flex justify-between items-center text-lg">
                  <span>Subtotal</span>
                  <span className="font-semibold">R$ {currentSale.subtotal.toFixed(2)}</span>
                </div>
                <div
