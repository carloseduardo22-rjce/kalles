...
public class UserService {
    ...
    public List<UserEntity> listAll() {
        return userRepository.findAllByTenantIdOrderByNameAsc(TenantContextHolder.requireTenantId());
    }

    @Transactional(readOnly = true)
    public UserEntity findById(UUID id) {
        return userRepository.findByIdAndTenantId(id, TenantContextHolder.requireTenantId())
                .orElseThrow(() -> new NotFoundException("User not found: " + id));
    }

    @Transactional(readOnly = true)
    public UserEntity findByEmail(String email) {
        return userRepository.findByTenantIdAndEmailIgnoreCase(TenantContextHolder.requireTenantId(), email)
