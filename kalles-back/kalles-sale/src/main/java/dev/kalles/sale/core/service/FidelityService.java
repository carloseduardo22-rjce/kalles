...
public class FidelityService {
    ...
    public FidelityResponse enrollClient(UUID clientId) {
        ...
        FidelityPolicy policy = fidelityPolicyRepository.findFirstByActiveTrue()
                .orElseThrow(() -> new IllegalStateException("Nenhuma política de fidelidade ativa encontrada."));
    ...
    public void rollbackSale(UUID clientId, BigDecimal discountToRestore, int pointsToDeduct) {
        ...
        if (discountToRestore.compareTo(BigDecimal.ZERO) > 0) {
            fidelity.rollbackDiscount(discountToRestore);
        }
        if (pointsToDeduct > 0) {
