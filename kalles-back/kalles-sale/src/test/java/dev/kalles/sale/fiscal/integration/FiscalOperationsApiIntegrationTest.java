package dev.kalles.sale.fiscal.integration;
...
import dev.kalles.sale.core.repository.PaymentRepository;
...
import dev.kalles.sale.fiscal.adapter.out.persistence.entity.FiscalDocumentEntity;
...
import dev.kalles.sale.fiscal.adapter.out.persistence.repository.SpringDataFiscalDocumentRepository;
import dev.kalles.sale.fiscal.adapter.out.persistence.repository.SpringDataFiscalIssuerAddressRepository;
import dev.kalles.sale.fiscal.adapter.out.persistence.repository.SpringDataFiscalIssuerProfileRepository;
...
class FiscalOperationsApiIntegrationTest extends AbstractCashRegisterApiSupport {
    ...
    private UUID seedSale() {
        ...
        sale.getItems().add(new dev.kalles.sale.core.entity.SaleItem(sale, product, 1, new BigDecimal("10.00")));
        return saleRepository.save(sale).getId();
    }

    private void seedRefund(UUID saleId) {
        Sale sale = saleRepository.findById(saleId).orElseThrow();
        paymentRepository.save(new dev.kalles.sale.core.entity.Payment(
                sale,
                dev.kalles.sale.core.enums.payment.PaymentMethod.CASH,
